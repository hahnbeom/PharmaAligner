...
def read_ligand(npz,key,debug=False):
    ...
    return (torch.tensor(data['xyz']).float(),
            ...
            np.array(data['names']), data['As'], data['Ds'], data['Rs'])

def find_searchlist(As,Ds,Rs,nPHcore):
    searchlist = []
    ...
    for a in As:
        for d in Ds:
            for r in Rs:
                searchlist.append(list(a)+list(d)+list(r))
...
def runner(inputf,key,opt):
    ...
    t0 = time.time()
    xlig,ylig,wy,atms,As_l,Ds_l,Rs_l = read_ligand(inputf, key, opt.debug)
        
    t1 = time.time()
    xPHcores, yPHcores = read_pharmacores(PHcoref) # text parsing

    nPHcore = {key:min(len(xPHcores[key]),len(idx)) for key,idx in zip(['A','D','R'],[As_l,Ds_l,Rs_l])}
    ...
    # ordered as A->D->R
    searchlist = find_searchlist(As_l,Ds_l,Rs_l,nPHcore)
    if opt.debug: print("found %d permutations"%len(searchlist))

    types = [0 for _ in range(nPHcore['A'])]+[1 for _ in range(nPHcore['D'])]+[2 for _ in range(nPHcore['R'])]
    # target
    xPHcores = torch.cat([xPHcores['A'],xPHcores['D'],xPHcores['R']],axis=0)
    yPHcores = torch.cat([yPHcores['A'],yPHcores['D'],yPHcores['R']],axis=0)
...
def multirunner(args):
    ...
    try:
        outputs,err = runner(inputf,key,opt)
    except:
